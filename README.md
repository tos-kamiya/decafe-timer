...
3. Style flags pick the ASCII layout: default multi-line (`Remaining` / `Expires at` + bar), `--one-line` (`HH:MM:SS ⣿⣿⣇⣀…`), or `--graph-only` (just the bar with no time stamp). `--bar-style` swaps the bar characters (`braille` default, or `blocks` for the previous look). They’re accepted on any invocation; when paired with `--run`, the live updates switch to that ASCII style instead of the Rich progress bar. Snapshots print `[You may drink coffee now.]` once the timer finishes.
